...
void upf_context_init(void)
{
    ...
    self.vxlan_info_hash = ogs_hash_make();
    ogs_assert(self.vxlan_info_hash);
...
void upf_context_final(void)
{
    ...
    ogs_assert(self.vxlan_info_hash);
    ogs_hash_destroy(self.vxlan_info_hash);
        ...
        uint8_t session_type, ogs_pfcp_pdr_t *pdr)
{
    ...
    if (sess->ipv4){
        ...
        if (NULL != vxlan_info){
            ...
            sess->support_vxlan_flag = true;
            sess->remote_interface_address = vxlan_info->remote_interface_address;
            sess->local_interface_address = vxlan_info->local_interface_address;
...
void upf_remoteserver_remove(upf_remoteserver_t *remoteserver)
{
    ...
    if (remoteserver->sctp.sock){
        ...
        //ogs_sctp_destroy(remoteserver->sctp.sock);//这里可以不释放,因为socket在nbrremoteserver_list中会释放
    }
