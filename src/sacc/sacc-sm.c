...
void sacc_state_initial(ogs_fsm_t *s, sacc_event_t *e)
{
    ...
    OGS_FSM_TRAN(s, &sacc_state_operational);
}
...
void sacc_state_operational(ogs_fsm_t *s, sacc_event_t *e)
{
    ...
    case OGS_EVENT_SBI_SERVER:
        ...
        CASE(OGS_SBI_SERVICE_NAME_ACC)
            ...
            CASE(OGS_SBI_RESOURCE_NAME_HANDSHAKE)
                sacc_handle_request(SACC_MSG_TYPE_HANDSHAKE, stream, &message);
                break;
            CASE(OGS_SBI_RESOURCE_NAME_HEARTBEAT)
                sacc_handle_request(SACC_MSG_TYPE_HEARDBEAT, stream, &message);
                break;             
            DEFAULT
                ogs_error("Invalid resource name [%s]",
    ...
    case OGS_EVENT_SBI_CLIENT:
        ...
        CASE(OGS_SBI_SERVICE_NAME_ACC)
            SWITCH(message.h.resource.component[0])
            CASE(OGS_SBI_RESOURCE_NAME_HANDSHAKE)
                sacc_handle_response(SACC_MSG_TYPE_HANDSHAKE, &message);
                break;
            CASE(OGS_SBI_RESOURCE_NAME_HEARTBEAT)
                sacc_handle_response(SACC_MSG_TYPE_HEARDBEAT, &message);
                break;             
            DEFAULT
                ogs_error("Invalid resource name [%s]",
                        message.h.resource.component[0]);
