...
static void sacc_main(void *data);
...
#define APP_NAME "sacc"
    rv = ogs_app_parse_local_conf(APP_NAME);
    if (rv != OGS_OK) return rv;
    sacc_metrics_init();

    ogs_sbi_context_init(OpenAPI_nf_type_SACC);
    sacc_context_init();
    ...
    thread = ogs_thread_create(sacc_main, NULL);
...
static void sacc_main(void *data)
{
    ...
    int rv;
setAffinity(5);
    ogs_fsm_init(&sacc_sm, sacc_state_initial, sacc_state_final, 0);
    ...
    for ( ;; ) {
        ...
        for ( ;; ) {
            ...
            ogs_assert(e);
            ogs_fsm_dispatch(&sacc_sm, e);
            ogs_event_free(e);
        }
    }
done:

    ogs_fsm_fini(&sacc_sm, 0);
}
