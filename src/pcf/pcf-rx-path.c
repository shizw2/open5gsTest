...
static void pcf_rx_asa_cb(void *data, struct msg **msg);
...
static int pcf_rx_fb_cb(struct msg **msg, struct avp *avp, 
...
int pcf_rx_send_asr(uint8_t *rx_sid, uint32_t abort_cause)
{
    ...
    /* Send the request */
    ret = fd_msg_send(&req, pcf_rx_asa_cb, svg);
    ogs_assert(ret == 0);
...
static int pcf_rx_str_cb( struct msg **msg, struct avp *avp, 
        struct session *sess, void *opaque, enum disp_action *act)
{
    int rv;
    int ret;
...
int pcf_rx_init(void)
{
    ...
    ret = fd_disp_register(pcf_rx_fb_cb, DISP_HOW_APPID, &data, NULL,
                &hdl_rx_fb);
    ogs_assert(ret == 0);

    /* Specific handler for AA-Request */
    data.command = ogs_diam_rx_cmd_aar;
    ret = fd_disp_register(pcf_rx_aar_cb, DISP_HOW_CC, &data, NULL,
                &hdl_rx_aar);
    ogs_assert(ret == 0);

    /* Specific handler for STR-Request */
    data.command = ogs_diam_rx_cmd_str;
    ret = fd_disp_register(pcf_rx_str_cb, DISP_HOW_CC, &data, NULL,
                &hdl_rx_str);
    ogs_assert(ret == 0);
