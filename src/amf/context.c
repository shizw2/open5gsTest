...
void change_network_name_from_utf8_2_ucs2(const char *c_network_name, uint8_t size, ogs_nas_network_name_t *network_full_name);
void amf_context_init(void)
{
    ...
    if (g_sps_id != 0)
    {
        ...
        ogs_pool_init(&icps_ue_pool, ogs_global_conf()->max.ue);
    ...
    while (ogs_yaml_iter_next(&root_iter)) {
        ...
        if (!strcmp(root_key, "amf")) {
            ...
            while (ogs_yaml_iter_next(&amf_iter)) {
                ...
                } else if (!strcmp(amf_key, "icps")) {
                    ogs_yaml_iter_t icps_array, icps_iter;
                    ogs_yaml_iter_recurse(&amf_iter, &icps_array);
                    do {
                        int family = AF_UNSPEC;
                        int i, num = 0;
                        const char *hostname[OGS_MAX_NUM_OF_HOSTNAME];
                        uint16_t port = self.icps_port;
                        const char *dev = NULL;
                        ogs_sockaddr_t *addr = NULL;
                        ...
                        if (ogs_yaml_iter_type(&icps_array) ==
                                YAML_MAPPING_NODE) {
                            memcpy(&icps_iter, &icps_array,
                                    sizeof(ogs_yaml_iter_t));
                        } else if (ogs_yaml_iter_type(&icps_array) ==
                            YAML_SEQUENCE_NODE) {
                            if (!ogs_yaml_iter_next(&icps_array))
                                break;
                            ogs_yaml_iter_recurse(&icps_array, &icps_iter);
                        } else if (ogs_yaml_iter_type(&icps_array) ==
                            YAML_SCALAR_NODE) {
                            break;
                        } else
                        ...
                        while (ogs_yaml_iter_next(&icps_iter)) {
                            const char *icps_key =
                                ogs_yaml_iter_key(&icps_iter);
                            ogs_assert(icps_key);
                            if (!strcmp(icps_key, "family")) {
                    ...
                    while (ogs_yaml_iter_next(&network_name_iter)) {
                        ...
                        if (!strcmp(network_name_key, "full")) {
                            ...
                            const char *c_network_name =
                                ogs_yaml_iter_value(&network_name_iter);
                            uint8_t size = strlen(c_network_name);
                            change_network_name_from_utf8_2_ucs2(c_network_name,size,network_short_name);
                            #if 0
                            uint8_t i;
                            for (i = 0;i<size;i++) {
                                /* Workaround to convert the ASCII to USC-2 */
...
void amf_ue_ran_ue_sps_icps_sync(amf_ue_t *amf_ue, ran_ue_t *ran_ue)
{
   ...
   // ran_ue = ran_ue_cycle(ran_ue);
	...
	tmsg.msg_type=INTERNEL_MSG_NGAP;
	tmsg.ran_ue_ngap_id=ran_ue->ran_ue_ngap_id;
	tmsg.amf_ue_ngap_id=ran_ue_find_by_id(amf_ue->ran_ue_id)->amf_ue_ngap_id;
...
icps_ue_spsno_t* icps_ue_add(char *supi)
{
    ...
    ogs_pool_id_calloc(&icps_ue_pool, &icps_ue);
    if (icps_ue == NULL) {
...
void icps_ue_remove(icps_ue_spsno_t *icps_ue)
{
    ...
    ogs_pool_id_free(&icps_ue_pool, icps_ue);
...
{
    return ogs_pool_find_by_id(&icps_ue_pool, id);
}
