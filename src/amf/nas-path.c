...
int nas_5gs_send_to_gnb(amf_ue_t *amf_ue, ogs_pkbuf_t *pkbuf)
{
    ...
    if(is_amf_sps())
    {
        ...
        if(amf_ue->next.m_tmsi){
            ...
            ran_ue_find_by_id(amf_ue->ran_ue_id)->m_tmsi=*(amf_ue->next.m_tmsi);
        }
        if(amf_ue->current.m_tmsi){
            ogs_info("nas_5gs_send_to_gnb,amf_ue->currunt.m_tmsi::%u\n",*(amf_ue->current.m_tmsi));
            ran_ue_find_by_id(amf_ue->ran_ue_id)->m_tmsi=*(amf_ue->current.m_tmsi);
        }
        ogs_info("ngap_send_to_ran_ue_sps!!!");
        return ngap_send_to_ran_ue_sps(ran_ue_find_by_id(amf_ue->ran_ue_id), pkbuf);
    }	
    else{
    ...
    ogs_expect(rv == OGS_OK);
        rv = ngap_send_to_ran_ue(ran_ue_find_by_id(amf_ue->ran_ue_id), pkbuf);
        ogs_expect(rv == OGS_OK);
...
int nas_5gs_send_de_registration_accept(amf_ue_t *amf_ue)
{
    ...
    if (amf_ue->nas.de_registration.switch_off == 0) {
        ...
        if (!gmmbuf) {
            ogs_error("gmm_build_de_registration_accept() failed");
            return OGS_ERROR;
        }
