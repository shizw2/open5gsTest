...
int ngap_send_to_ran_ue_sps(ran_ue_t *ran_ue, ogs_pkbuf_t *pkbuf)
{
	...
	ogs_info("ngap_send_to_ran_ue_sps len:%d,pkbuf->len=%d,sizeof(tmsg.msg_head)=%lu",len,pkbuf->len,sizeof(tmsg.msg_head));
        ...
        int rv = ngap_send_to_gnb(gnb, pkbuf, ran_ue->gnb_ostream_id);
        ogs_expect(rv == OGS_OK);
				...
				NGAP_ProcedureCode_t procedureCode, size_t nas_len,uint8_t *nasPdu)
{
    ...
    switch(sh->security_header_type) {
        ...
        case OGS_NAS_SECURITY_HEADER_INTEGRITY_PROTECTED_AND_CIPHTERD_WITH_NEW_INTEGRITY_CONTEXT:
            ...
            break;
        default:
            ogs_error("Not implemented(security header type:0x%x)", sh->security_header_type);
            return OGS_ERROR;
    }
    //printf("========================================================nasPdu->size:%lu\n",nas_len);
    if (ran_ue->amf_ue){
        if (nas_5gs_security_decode(ran_ue->amf_ue, security_header_type, nasbuf) != OGS_OK){
            ogs_error("nas_eps_security_decode failed()");
            return OGS_ERROR;
        }
    }
    ...
    if (h->extended_protocol_discriminator == OGS_NAS_EXTENDED_PROTOCOL_DISCRIMINATOR_5GMM){
        ...
        return rv;
    } else if (h->extended_protocol_discriminator == OGS_NAS_EXTENDED_PROTOCOL_DISCRIMINATOR_5GSM) {
