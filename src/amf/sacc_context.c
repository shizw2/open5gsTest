        ...
        const char *local, const char *nrf, const char *scp, ogs_sbi_nf_instance_t *nf_instance)
{
    ...
    while (ogs_yaml_iter_next(&root_iter)) {
        ...
        if (local && !strcmp(root_key, local)) {
            ogs_yaml_iter_t node_iter;
            ogs_yaml_iter_recurse(&root_iter, &node_iter);
...
// int sacc_sbi_context_get_nf_info3(
...
//                 } else
//                         ogs_warn("unknown key `%s`", node_key);
//                 }
//             } while (ogs_yaml_iter_type(&node_array) ==
//                     YAML_SEQUENCE_NODE);
