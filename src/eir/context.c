    ...
    while (ogs_yaml_iter_next(&root_iter)) {
        ...
        if (!strcmp(root_key, "eir")) {
            ...
            while (ogs_yaml_iter_next(&eir_iter)) {
                ...
                ogs_assert(eir_key);
                if (!strcmp(eir_key, "sbi")) {
                    /* handle config in sbi library */
                } else if (!strcmp(eir_key, "service_name")) {
                    /* handle config in sbi library */
                } else if (!strcmp(eir_key, "discovery")) {
                    /* handle config in sbi library */
                } else if (!strcmp(eir_key, "metrics")) {
                    /* handle config in metrics library */
                } else if (!strcmp(eir_key, "info")) { //smf,amf的info都是单独解析的，不太合适放到某个library中
                    ogs_sbi_nf_instance_t *nf_instance = NULL;

                    ogs_yaml_iter_t info_array, info_iter;
                    ogs_yaml_iter_recurse(&eir_iter, &info_array);
                    ...
                    do {
                        ...
                        ogs_sbi_eir_info_t *eir_info = NULL;
                        ...
                        while (ogs_yaml_iter_next(&info_iter)) {
                            ...
                            if (!strcmp(info_key, "supi")) {
                                is_nfinfo_changed = ogs_sbi_context_parse_supi_ranges(&info_iter, &eir_info->supiRanges);
                            } else
