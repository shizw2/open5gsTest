...
int ogs_sbi_discover_and_send(ogs_sbi_xact_t *xact)
{
    ...
    ogs_info("test:ogs_sbi_discover_and_send sbi_object->type:%d,service_type:%s,supi_id:%s, routingIndicator:%s.",sbi_object->type,ogs_sbi_service_type_to_name(service_type), xact->supi_id, xact->routingIndicator); 
    ...
    if (nf_instance){
        ogs_info("test:ogs_sbi_discover_and_send,request->h.uri:%s, nf_instance id:%s,service_type:%s,target_nf_type:%s.",request->h.uri,nf_instance->id,ogs_sbi_service_type_to_name(service_type),OpenAPI_nf_type_ToString(target_nf_type));
    }
    /* Target Client */
    ...
    if (scp_client) {
        ...
        if (client) {
            ...
            if (discovery_option &&
                ...
                ogs_info("test:set target_nf_instance_id1:%s",discovery_option->target_nf_instance_id);
                ogs_sbi_header_set(request->http.headers,
                        OGS_SBI_CUSTOM_DISCOVERY_TARGET_NF_INSTANCE_ID,
                        discovery_option->target_nf_instance_id);
            } else if (nf_instance && nf_instance->id) {
                ogs_info("test:set target_nf_instance_id2:%s",discovery_option->target_nf_instance_id);
                ogs_sbi_header_set(request->http.headers,
                        OGS_SBI_CUSTOM_DISCOVERY_TARGET_NF_INSTANCE_ID,
                        nf_instance->id);
            }
        ...
        ogs_info("test:client instance is available, use direct communication");
         ...
         **********************************************/
        ogs_info("test:No SCP and Client, Use NRF for NF-Discovery");
