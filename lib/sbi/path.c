...
int ogs_sbi_server_handler(ogs_sbi_request_t *request, void *data)
{
    ...
    ogs_info("*test:%p headers cnt:%d,params cnt:%d,request:%s method:%s",request,ogs_hash_count(request->http.headers),ogs_hash_count(request->http.params),request->http.content,request->h.method);
