...
if [ "$SYSTEM" = "Linux" ]; then
    ...
    if test "x`sysctl -n net.ipv6.conf.ogstun2.disable_ipv6`" = x1; then
        echo "net.ipv6.conf.ogstun2.disable_ipv6=0" > /etc/sysctl.d/30-open5gs.conf
        sysctl -p /etc/sysctl.d/30-open5gs.conf
    fi
    ip addr del 10.46.0.1/16 dev ogstun2 2> /dev/null
    ip addr add 10.46.0.1/16 dev ogstun2
    #ip addr del 2001:db8:cafe::1/48 dev ogstun2 2> /dev/null
    #ip addr add 2001:db8:cafe::1/48 dev ogstun2
